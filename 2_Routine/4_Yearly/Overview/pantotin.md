---
dg-publish: true
dg-permalink: 2_Routine/4_Yearly/Overview/pantotin
tags:
  - yearly
  - pantotin
  - overview
...
# Pantotin
```dataviewjs
dv.span("** 💊 Pantotin **") /* optional ⏹️💤⚡⚠🧩↑↓⏳📔💾📁📝🔄📝🔀⌨️🕸️📅🔍✨ */
const calendarData = {
...
//DataviewJS loop
for (let page of dv.pages('"2_Routine/1_Daily/Daily-Routine"').where(p => p.pantotin)) {
    //dv.span("<br>" + page.file.name) // uncomment for troubleshooting
    calendarData.entries.push({
        date: page.file.name,     // (required) Format YYYY-MM-DD
        intensity: page.pantotin, // (required) the data you want to track, will map color intensities automatically
